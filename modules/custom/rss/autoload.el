;;; custom/rss/autoload.el -*- lexical-binding: t; -*-
...
`fill-column' and centering it with `visual-fill-column-mode'."
  (let ((inhibit-read-only t)
    ...
    (setq-local shr-current-font doom-variable-pitch-font)
...
(defun +rss-popup-pane (buf)
  ...
  (select-window (display-buffer buf '((window-width . nil)))))
...
(defun elfeed-search-print-entry-titlelast (entry)
  ...
  (let* ((date (elfeed-search-format-date (elfeed-entry-date entry)))
         ...
         (feed-column (elfeed-format-column
                       feed-title (elfeed-clamp
                                   elfeed-search-feed-min-width
                                   elfeed-search-feed-min-width
                                   elfeed-search-feed-max-width)
                       :left))
         (tags-column (elfeed-format-column
                       tags-str (elfeed-clamp
                                 elfeed-search-tag-min-width
                                 elfeed-search-tag-min-width
                                 elfeed-search-tag-max-width)
                       :left)))
    (when tags-column (insert (propertize tags-column 'face 'elfeed-show-tag-face)) " ")
    (insert (propertize date 'face 'elfeed-search-date-face) " ")
    (when feed-column (insert (propertize feed-column 'face 'elfeed-search-feed-face) " "))
...
;;;###autoload
(defun elfeed-search-star (entry)
  "Remove/add TAG unread from the current entry"
  (interactive (list (elfeed-search-selected :ignore-region)))
  (when (elfeed-entry-p entry)
...
;;;###autoload
(defun elfeed-show-star ()
  "Apply TAG star to current entry."
  (interactive)
  (let ((tag 'star))
    (elfeed-show-tag tag)))

;;;###autoload
(defun elfeed-show-mark-read ()
  "Remove TAG unread from current entry."
  (interactive)
  (let ((tag 'unread))
