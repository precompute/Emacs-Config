...
(defvar +doom-dashboard-name "*front*"
  "The name to use for the dashboard buffer.")

(defvar +doom-dashboard-functions
  '(doom-dashboard-widget-banner
    doom-dashboard-custom-text)
...
(defface doom-dashboard-loaded '((t (:inherit menu)))
...
(define-derived-mode +doom-dashboard-mode special-mode
  (format "Emacs %s %s @ %s" emacs-version emacs-repository-branch (seq-take emacs-repository-version 6))
...
(defun doom-dashboard-custom-text ()
  (mapc (lambda (line)
          (insert (propertize (+doom-dashboard--center +doom-dashboard--width line)
                              'face 'variable-pitch) " ")
          (insert "\n"))
